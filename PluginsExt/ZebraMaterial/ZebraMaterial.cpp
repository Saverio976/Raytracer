...
namespace RayTracer::PluginsExt::Zebra {
    ZebraMaterial::ZebraMaterial(const Scenes::ISetting &config):
    ...
    Images::Color ZebraMaterial::getColor(const Images::Ray &ray, const Entities::Transform::ITransform &centerObj, const Entities::Transform::Vector3f &intersect, const Scenes::IDisplayable &displayable) const
    {
        double boardX = intersect.getX() / this->_size;
        double boardY = intersect.getY() / this->_size;
        double boardZ = intersect.getZ() / this->_size;
        int result = static_cast<int>(boardX + boardY + boardZ);
    ...
    void ZebraMaterial::setColor(const Images::Color &color)
    {
        _pair = color;
    }

    Images::Color ZebraMaterial::redirectionLight(const Images::Ray &ray, const Scenes::IDisplayable &displayable,
